/*
    CREDITS TO BRICKWALL FOR THIS 
    SOURCE: BOTCHED FRAMEWORK
*/
local PANEL = {}
...
function PANEL:Init()
    ...
    self.header.Paint = function(self2, w, h)
        if (!self.headerShouldDraw) then return end
...
function PANEL:OnChildAdded(panel)
    if (!self.initFinished) then return end
...
vgui.Register("pb_popup_base", PANEL, "DFrame")